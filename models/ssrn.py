...
class SSRN(nn.Module):
    ...
    def forward(self, x):
        ...
        #x = F.avg_pool2d(x, x.size()[-1].item())
        x = F.avg_pool2d(x, x.size()[-1])
        x = self.fc(x.squeeze())
        
        return x
    
    def flops(self, shape=(1, 48, 9, 9), verbose=True):
...
def ssrn(dataset, patch_size):
    model = None
    if dataset == 'sa':
        model = SSRN(num_classes=16)
    elif dataset == 'pu':
        model = SSRN(num_classes=9)
    elif dataset == 'whulk':
        model = SSRN(num_classes=9, k=132)
    elif dataset == 'hrl':
        model = SSRN(num_classes=14)
    elif dataset == 'ip':
        model = SSRN(num_classes=16, k=97)
    elif dataset == 'hu2018':
        model = SSRN(num_classes=20, k=21)
    elif dataset == 'whu-ohs':
        model = SSRN(num_classes=24, k=13)
    return model
