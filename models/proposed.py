...
class MyTransformer(nn.Module):
    ...
    def flops(self, shape=(1, 48, 7, 7), verbose=True):
...
def proposed(model_config, dataset, patch_size):
    model = None
    if dataset == 'sa':
        ...
        #model = MyTransformer(img_size=patch_size, in_chans=200, num_classes=16, n_groups=[2, 2, 2], depths=[1, 2, 1])
    elif dataset == 'hu2018':
        model = MyTransformer(img_size=patch_size, in_chans=48, num_classes=20, n_groups=[4, 4, 4], depths=[1, 2, 1])
    elif dataset == 'whu-ohs':
        model = MyTransformer(img_size=patch_size, in_chans=32, num_classes=24, n_groups=[4, 4, 4], depths=[1, 2, 1])
    return model

if __name__ == "__main__":
    net = MyTransformer(img_size=7, in_chans=48, num_classes=20, n_groups=[4, 4, 4], depths=[1, 2, 1])
    net.flops(verbose=True)
