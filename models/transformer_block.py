...
class Fusion_selective_woshoutcut(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.dim = dim
        inter_dim = max(dim, 32)
    ...
    def forward(self, x, y):
        ...
        feats = initial.transpose(1,2)
        gap = self.pool(feats).unsqueeze(3)
        w_atten = self.fc2(self.fc1(gap)).squeeze()
        w_atten = w_atten.unsqueeze(1)
        w_conv = self.fc3(self.fc1(gap)).squeeze()
        w_conv = w_conv.unsqueeze(1)
        w = torch.cat((w_atten, w_conv), dim=1)
        w = self.softmax(w)
        result = w[:, 0].unsqueeze(1) * x + w[:, 1].unsqueeze(1) * y
        return result, 1 - w_conv, w_conv

class Fusion_selective(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.dim = dim
        inter_dim = max(dim, 32)
    ...
    def forward(self, x, y):
        ...
        w = torch.cat((w_atten, w_conv), dim=1)
        w = self.softmax(w)
        result = w[:, 0].unsqueeze(1) * x + w[:, 1].unsqueeze(1) * y + initial
        return result, 1 - w_conv, w_conv
