...
class PreNorm(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        if self.fn.__class__.__name__ == 'LSTM':
            return self.fn(self.a_2 * (x - mean) / (std + self.eps) + self.b_2, **kwargs)[0]
...
class SMSA(nn.Module):
    ...
    def spe_mask(self, x):
        ...
        #         plt.title('batch 2')
        #         plt.show()
        return spe_pairwise_weights.to(x.device)
    ...
    def spa_mask(self, x):
        ...
        return spa_pairwise_weights.to(x.device)
    ...
    def forward(self, x):
        ...
        dots = dots.to(x.device) * spe_mask.to(x.device) * spa_mask.to(x.device)  # acc:94.688, flip_aug:95.295
...
class multiTrans(nn.Module):
    ...
    def forward(self, x):  # original x (b 1 c h w)
        ...
        # x1r = multiscan(x, 1).to(x.device)
        # x2r = multiscan(x, 2).to(x.device)
        # x3r = multiscan(x, 3).to(x.device)
        # x4r = multiscan(x, 4).to(x.device)
        # x5r = multiscan(x, 5).to(x.device)
        # x6r = multiscan(x, 6).to(x.device)
        # x7r = multiscan(x, 7).to(x.device)
        # x8r = multiscan(x, 8).to(x.device)  # (b c s)
        ...
        x8r = x8rt.to(x.device)# batch, fea dim, steps
    ...
    def flops(self, shape=(1, 270, 7, 7), verbose=True):
...
def msrt(model_config):
    model = multiTrans(**model_config)
    return model

if __name__ == '__main__':
    model = msrt(model_config={'input_channels': 48, 'n_classes': 20, 'patch_size': 11})
    import copy, fvcore.nn.flop_count as flop_count, fvcore.nn.parameter_count as parameter_count
